...
def evaluate_multi_target_predictor(
    ...
    if strategy == "conservative":
        ...
        print(f"  Bias weight: {bias_weight}")
    ...
    for question_data in tqdm(test_data, desc="Evaluating"):
        ...
        with torch.no_grad():
            if strategy == "argmax":
                ...
                predicted_step = predictor.predict_biased(h0_batch, bias_weight=bias_weight)[0].item()
            else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="argmax",
        choices=["argmax", "conservative", "sample", "ensemble", "biased"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Logit bias weight for test-time control (biased strategy)",
    )
    parser.add_argument(
        "--max_questions",
        type=int,
